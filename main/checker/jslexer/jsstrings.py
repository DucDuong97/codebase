...
template_string_characters_re = r'([^\`\\]+)|(\\{EscapeSequence})|{LineContinuation}'\
                                     .format(LineContinuation=line_continuation_re,
                                         EscapeSequence=escape_sequence_re)

string_literal_re           = r'\"({DoubleStringCharacters})*\"|\'({SingleStringCharacters})*\'|\`({TemplateStringCharacters})*\`'\
                                    .format(SingleStringCharacters=single_string_characters_re,
                                        DoubleStringCharacters=double_string_characters_re,
                                        TemplateStringCharacters=template_string_characters_re)
