...
def URI_value(x):
    ...
    return cssast.Uri(url)

def STRING_value(x):
    q = x[0]
    return cssast.String(x[1:-1].replace(u'\\'+q,q))
...
class cssparser(object):
    ...
    def p_stylesheet(self, p):
        ...
        '''
        if isinstance(p[1], cssast.Charset):
            p[0] = cssast.Stylesheet(p[4], p[3], p[1])
        else:
            p[0] = cssast.Stylesheet(p[3], p[2])
        print(repr(p[0]))
    ...
    def p_page(self, p):
        ...
        '''
        if isinstance(p[3], cssast.Ident):
            p[0] = cssast.Page(p[6], p[3])
        else:
            p[0] = cssast.Page(p[4])
    ...
    def p_block_declarations(self, p):
        ...
        block_declarations : block_declarations ';' spaces declaration
                           | block_declarations ';' spaces directive
                           | declaration
                           | directive
        '''
        if len(p) == 2:
    ...
    def p_declaration(self, p):
        ...
        declaration : property ':' spaces expr prio
                    ...
                    | property ':' spaces directive
                    | empty
        '''
        if len(p) == 2:
            p[0] = None
        elif len(p) == 1:
            p[0] = cssast.Declaration(p[1], '', False)
        else:
            important = len(p) == 6
            p[0] = cssast.Declaration(p[1], p[4], important)

    def p_property(self, p):
        '''
        property : IDENT spaces
        '''
        p[0] = cssast.Ident(p[1])
    ...
    def p_directive(self, p):
        '''
        directive : DIRECTIVE spaces
        '''
        p[0] = cssast.Directive(p[1])
    ...
    def p_expr(self, p):
        ...
        '''
        if len(p) == 4:
            p[0] = u''.join([str(x) for x in p[1:]])
        elif len(p) == 3:
            p[0] = str(p[1]) + u' ' + str(p[2])
        else:
            p[0] = p[1]
    ...
    def p_term(self, p):
        ...
        term : unary_operator term_quant spaces
             ...
             | function
        '''
        if isinstance(p[1], cssast.Function) or isinstance(p[1], cssast.Hexcolor):
            p[0] = p[1]
    ...
    def p_function(self, p):
        ...
        p[0] = cssast.Function(name, p[3])

    def p_hexcolor(self, p):
        '''
        hexcolor : HASH spaces
        '''
        p[0] = cssast.Hexcolor(p[1])
